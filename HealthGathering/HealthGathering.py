...
class VizDoomEnv(Env):
    def __init__(self, render=False):
        super().__init__()
        self.game = DoomGame()
        self.game.load_config("../ViZDoom-master/scenarios/health_gathering.cfg")
...
def train():
    ...
    # policy_kwargs = dict(net_arch=[64, 128, 128, 64, dict(vf=[128, 128], pi=[32, 32])])
    policy_kwargs = dict(net_arch=[128, 64, dict(vf=[64, 32], pi=[32, 16])])

    model = PPO('CnnPolicy', env, tensorboard_log=LOG_DIR, verbose=1, learning_rate=0.0001, n_steps=2048,
                batch_size=64, n_epochs=10, gamma=0.99, policy_kwargs=policy_kwargs)
    print(model.policy)
...
def makeGif():
    ...
    imageio.mimsave('healthGathering.gif', [np.array(img) for i, img in enumerate(images) if i % 2 == 0], fps=15)
