...
class VizDoomEnv(Env):
    def __init__(self, render=False):
        super().__init__()
        self.game = DoomGame()
        self.game.load_config("../ViZDoom-master/scenarios/defend_the_line.cfg")
...
def test():
    model = PPO.load("./train/model50000.zip")
    env = VizDoomEnv(True)
...
def makeGif():
    ...
    imageio.mimsave('defendTheLine.gif', [np.array(img) for i, img in enumerate(images) if i % 2 == 0], fps=15)
