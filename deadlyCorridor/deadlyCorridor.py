...
class VizDoomEnv(Env):
    def __init__(self, render=False):
        super().__init__()
        self.game = DoomGame()
        self.game.load_config("../ViZDoom-master/scenarios/deadly_corridor.cfg")
...
def train():
    ...
    model.learn(40000, callback=callback)

    env.close()


def test():
    model = PPO.load("./train/model40000.zip")
    env = VizDoomEnv(True)
...
def makeGif():
    ...
    imageio.mimsave('deadlyCorridor.gif', [np.array(img) for i, img in enumerate(images) if i % 2 == 0], fps=15)
